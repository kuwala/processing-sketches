...
void draw() {
  ...
  if(millis() - lastTime > timeLimit) { // maybe here
    
    if(drawOrErase == 0){
      // erase
      if(antBoards.size() > 0) {
      antBoards.remove(antBoards.size()-1);
      } else {
        drawOrErase =1;
      }
    } else {
...
void mouseClicked() {
  ...
  antBoards.add(new LangtonsAnt((mouseX / 10) * 10, (mouseY / 10) * 10, newBoardSize, newBoardSize));
